...
def run_phi(data, **kwargs):
    ...
    with basic_model:
        ...
        try:
            ...
            start = trace[0]
            trace = sample(NUM_OF_ITERATIONS, tune=round(NUM_OF_ITERATIONS/2),chains=8, init=None, step=step, start=start,progressbar=verbose,random_seed=seed)    
    ...
    #        trace = sample(NUM_OF_ITERATIONS, progressbar=verbose,random_seed=123, njobs=njobs,init=None,tune=100)     
            ...
            step = Metropolis()
            #start = find_MAP()
            trace = sample(NUM_OF_ITERATIONS, progressbar=verbose,random_seed=seed,step=step)#,start=start)
...
def main(args=None):
    ...
    as_mat =  df.values
    result = run_phi(as_mat,limits=limits,verbose=True,keep_missing=True,N=25000)
    print(result)
